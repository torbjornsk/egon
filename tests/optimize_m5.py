"""
Optimize M5 parameters using the test suite
...
def optimize_m5():
    ...
    for i, (rsi_buy, rsi_sell, rsi_exit_long, rsi_exit_short, profit_target) in enumerate(combinations, 1):
        ...
        period_results = suite.test_config(config_name, test_config, timeframe='M5', use_cache=True)
    ...
    by_pf = sorted(results, key=lambda x: x['avg_pf'], reverse=True)
    ...
    print("="*100)
    print(f"{'Rank':<6} {'Config':<45} {'PF':<10} {'Avg Ret':<10} {'Trades':<8} {'WR':<8} {'Sharpe':<8}")
    print("-"*100)
    
    for i, r in enumerate(by_pf[:10], 1):
        print(f"{i:<6} {r['config_name']:<45} {r['avg_pf']:>8.2f} {r['avg_return']:>8.1f}% "
              f"{r['avg_trades']:>7.0f} {r['avg_wr']:>6.1f}% {r['avg_sharpe']:>6.2f}")
    ...
    # Find best balanced config (top in multiple categories)
    best = by_avg_return[0]
    ...
    if improvement > 2:
        print(f"\n✓ Recommended: Update config/m5_params.json with optimized parameters")
    elif improvement > 0:
    ...
    with open('tests/m5_optimization_results.json', 'w') as f:
        json.dump({
            'top_10_by_return': [
                {
                    'config': r['config'],
    ...
    print(f"\nDetailed results saved to: tests/m5_optimization_results.json")
