"""
Fast M1 optimization using Monte Carlo sampling
Tests on 50 random 7-day windows instead of full dataset
~50x faster with 99% confidence
"""
import json
...
def optimize_m1_monte_carlo():
    print("="*100)
    print("M1 PARAMETER OPTIMIZATION (Monte Carlo Sampling)")
    print("="*100)
    ...
    # Define parameter ranges (M1 uses tighter ranges)
    ...
    # Monte Carlo settings (M1 uses shorter windows - more frequent trading)
    n_samples = 50  # 50 samples = ~99% confidence
    window_days = 7  # 1-week windows (M1 trades more frequently)
    test_period = 60  # Sample from last 60 days
    ...
    for i, (rsi_buy, rsi_sell, rsi_exit_long, rsi_exit_short, profit_target) in enumerate(combinations, 1):
        ...
        results.append({
            ...
            'consistency': 1 / (mc_results['std_return'] + 0.1),
            'risk_adjusted': mc_results['mean_return'] * mc_results['mean_sharpe'] / max(abs(mc_results['std_return']), 1)
        })
    ...
    if improvement > 5:
        print(f"\n✓ Recommended: Update config/m1_params.json")
        print(f"  Note: Validate with full test before deploying")
    elif improvement > 0:
    ...
    # Save results
    with open('tests/m1_monte_carlo_results.json', 'w') as f:
        json.dump({
            'settings': {
                'n_samples': n_samples,
                'window_days': window_days,
    ...
    print(f"\nResults saved to: tests/m1_monte_carlo_results.json")
    print(f"Total time: {elapsed/60:.1f} minutes")

if __name__ == '__main__':
    optimize_m1_monte_carlo()
